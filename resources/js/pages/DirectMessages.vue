  ...
  <AppLayout :breadcrumbs="breadcrumbs">
    <div class="flex-1 w-full bg-surface-container-lowest border border-outline-variant rounded-xl shadow-sm overflow-hidden flex h-[calc(100vh-200px)] min-h-[500px]">
      ...
      <div class="w-80 sm:w-96 border-r border-outline-variant flex flex-col bg-surface-container-lowest shrink-0">
        <div class="p-4 border-b border-outline-variant flex flex-col gap-3 dark:bg-inverse-surface">
          <h2 class="text-xl font-bold text-on-surface select-none dark:text-white">Messages</h2>
          <div class="relative">
        ...
        <div class="flex-1 overflow-y-auto p-2 divide-y divide-outline-variant/30 dark:bg-inverse-surface">
          <div
            ...
            :class="[
              ...
              activeContact?.id === contact.id ? 'bg-primary/5' : 'hover:bg-surface-container-low dark:bg-inverse-surface'
            ]"
          >
            ...
            <div class="flex-1 min-w-0">
              <div class="flex justify-between items-baseline mb-0.5">
                <h4 :class="['font-label-md truncate transition-colors group-hover:text-primary', activeContact?.id === contact.id ? 'text-primary font-bold' : 'text-on-surface font-semibold dark:text-white']">
                  {{ contact.name }}
                </h4>
                <span v-if="contact.last_message_time" class="text-[10px] text-on-surface-variant font-medium select-none shrink-0 dark:text-surface-variant">{{ contact.last_message_time }}</span>
              </div>
              <p class="text-xs text-on-surface-variant truncate dark:text-surface-variant">{{ contact.last_message || contact.role }}</p>
            </div>
      ...
      <div class="flex-1 flex flex-col bg-surface-container-low overflow-hidden">
        ...
        <div v-if="!activeContact" class="flex-1 flex flex-col items-center justify-center text-on-surface-variant p-6 select-none dark:bg-inverse-surface">
          ...
          <h3 class="text-lg font-bold text-on-surface mb-1 dark:text-white">Direct Messages</h3>
          <p class="text-sm max-w-sm text-center dark:text-white">Select a colleague from the list on the left to start a private conversation.</p>
        </div>

        <div v-else class="flex-1 flex flex-col overflow-hidden">
          <div class="bg-surface-container-lowest border-b border-outline-variant p-4 flex items-center gap-3 shrink-0 dark:bg-inverse-surface">
            ...
            <div class="min-w-0">
              <Link :href="`/profile/${activeContact.id}`" class="font-bold text-on-surface hover:text-primary transition-colors truncate block no-underline font-semibold text-lg dark:text-white">{{ activeContact.name }}</Link>
              <p class="text-xs text-on-surface-variant truncate dark:text-surface-variant">
                {{ activeContact.role }} • <span :class="activeContact.status === 'online' ? 'text-green-600 font-semibold' : 'text-on-surface-variant dark:text-surface-variant'">{{ activeContact.status === 'online' ? 'Online' : 'Offline' }}</span>
              </p>
            </div>
          </div>

          <div ref="messagesContainer" class="flex-1 overflow-y-auto p-6 space-y-4 flex flex-col dark:bg-inverse-surface">
            <div
            ...
            <div v-if="messages.length === 0" class="text-center py-16 text-on-surface-variant text-xs select-none dark:text-surface-variant">
