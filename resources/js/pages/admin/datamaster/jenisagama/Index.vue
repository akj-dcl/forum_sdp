...
function destroyAgama(id: number) {
  if (!confirm('Yakin mau hapus Jenis Agama ini?')) return
  router.delete(`/admin/jenis-agamas/${id}`)
}
</script>

<template>
  <Head title="Master Jenis Agama" />

  <AppLayout>
    <div class="flex h-full flex-1 flex-col gap-4 p-4 md:p-6 bg-background text-foreground">
      <div class="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
        <div>
          <h1 class="text-2xl font-semibold tracking-tight">Master Jenis Agama</h1>
          <p class="text-sm text-muted-foreground">Kelola klasifikasi jenis tindak agama Warga Binaan Pemasyarakatan.</p>
        </div>

        <Link
          v-if="can('jenis-agamas.create')"
          href="/admin/jenis-agamas/create"
          ...
          <table class="w-full caption-bottom text-sm">
            <thead class="[&_tr]:border-b border-border">
              <tr class="border-b transition-colors hover:bg-muted/50 data-[state=selected]:bg-muted">
                <th class="h-12 px-4 text-left align-middle font-medium text-muted-foreground w-3/4">Nama Agama</th>
                <th class="h-12 px-4 text-right align-middle font-medium text-muted-foreground">Aksi</th>
              </tr>
            </thead>
            <tbody class="[&_tr:last-child]:border-0">
              <tr
                v-for="k in agamas.data"
                :key="k.id"
                class="border-b border-border transition-colors hover:bg-muted/50"
              >
                <td class="p-4 font-medium">{{ k.nama_agama }}</td>
                <td class="p-4 text-right align-middle">
                  <div class="flex justify-end gap-2">
                    <Link
                      v-if="can('jenis-agamas.edit')"
                      :href="`/admin/jenis-agamas/${k.id}/edit`"
                    ...
                    <button
                      v-if="can('jenis-agamas.delete')"
                      type="button"
                      @click="destroyAgama(k.id)"
              ...
              <tr v-if="!agamas.data.length">
                <td colspan="2" class="p-8 text-center text-muted-foreground">
                  Tidak ada data Jenis Tindak Agama ditemukan.
                </td>
              </tr>
            </tbody>
          </table>
      ...
      <div v-if="agamas.links.length > 3" class="flex items-center justify-between mt-2">
        <div class="text-sm text-muted-foreground">
          Menampilkan {{ agamas.from ?? 0 }} sampai {{ agamas.to ?? 0 }} dari {{ agamas.total }} data
        </div>
        <div class="flex gap-1">
          <template v-for="(link, key) in agamas.links" :key="key">
