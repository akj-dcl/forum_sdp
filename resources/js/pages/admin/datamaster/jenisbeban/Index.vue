...
function destroyBeban(id: number) {
  if (!confirm('Yakin mau hapus Jenis Beban ini?')) return
  router.delete(`/admin/jenis-bebans/${id}`)
}
</script>

<template>
  <Head title="Master Jenis Beban" />

  <AppLayout>
    <div class="flex h-full flex-1 flex-col gap-4 p-4 md:p-6 bg-background text-foreground">
      <div class="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
        <div>
          <h1 class="text-2xl font-semibold tracking-tight">Master Jenis Beban</h1>
          <p class="text-sm text-muted-foreground">Kelola klasifikasi jenis tindak beban Warga Binaan Pemasyarakatan.</p>
        </div>

        <Link
          v-if="can('jenis-bebans.create')"
          href="/admin/jenis-bebans/create"
          ...
          <table class="w-full caption-bottom text-sm">
            <thead class="[&_tr]:border-b border-border">
              <tr class="border-b transition-colors hover:bg-muted/50 data-[state=selected]:bg-muted">
                <th class="h-12 px-4 text-left align-middle font-medium text-muted-foreground w-3/4">Nama Beban</th>
                <th class="h-12 px-4 text-right align-middle font-medium text-muted-foreground">Aksi</th>
              </tr>
            </thead>
            <tbody class="[&_tr:last-child]:border-0">
              <tr
                v-for="k in bebans.data"
                :key="k.id"
                class="border-b border-border transition-colors hover:bg-muted/50"
              >
                <td class="p-4 font-medium">{{ k.nama_beban }}</td>
                <td class="p-4 text-right align-middle">
                  <div class="flex justify-end gap-2">
                    <Link
                      v-if="can('jenis-bebans.edit')"
                      :href="`/admin/jenis-bebans/${k.id}/edit`"
                    ...
                    <button
                      v-if="can('jenis-bebans.delete')"
                      type="button"
                      @click="destroyBeban(k.id)"
              ...
              <tr v-if="!bebans.data.length">
                <td colspan="2" class="p-8 text-center text-muted-foreground">
                  Tidak ada data Jenis Tindak Beban ditemukan.
                </td>
              </tr>
            </tbody>
          </table>
      ...
      <div v-if="bebans.links.length > 3" class="flex items-center justify-between mt-2">
        <div class="text-sm text-muted-foreground">
          Menampilkan {{ bebans.from ?? 0 }} sampai {{ bebans.to ?? 0 }} dari {{ bebans.total }} data
        </div>
        <div class="flex gap-1">
          <template v-for="(link, key) in bebans.links" :key="key">
