...
function submit() {
  form.post('/admin/jenis-golongans')
}
</script>

<template>
  <Head title="Tambah Jenis Golongan" />

  <AppLayout>
    <div class="flex h-full flex-1 flex-col gap-4 p-4 md:p-6 bg-background text-foreground">
      <div class="flex flex-col gap-2 md:flex-row md:items-center md:justify-between">
        <div>
          <h1 class="text-2xl font-semibold tracking-tight">Tambah Jenis Tindak Golongan</h1>
          <p class="text-sm text-muted-foreground">Masukkan klasifikasi tindak golongan baru.</p>
        </div>

        <Link
          href="/admin/jenis-golongans"
        ...
        <form @submit.prevent="submit" class="p-6 space-y-6">
          <div class="grid gap-6 md:grid-cols-2">
            <div class="md:col-span-2 space-y-2">
              <label class="text-sm font-medium leading-none">Nama Golongan</label>
              <input
              ...
              />
              <div v-if="form.errors.nama_golongan" class="text-sm font-medium text-destructive">{{ form.errors.nama_golongan }}</div>
            </div>
          </div>
