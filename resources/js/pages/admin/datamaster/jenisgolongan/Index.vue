...
function destroyGolongan(id: number) {
  if (!confirm('Yakin mau hapus Jenis Golongan ini?')) return
  router.delete(`/admin/jenis-golongans/${id}`)
}
</script>

<template>
  <Head title="Master Jenis Golongan" />
  ...
  <AppLayout>
    <div class="flex h-full flex-1 flex-col gap-4 p-4 md:p-6 bg-background text-foreground dark:bg-inverse-surface">
      <div class="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
        ...
        <Link
          v-if="can('jenis-golongans.create')"
          href="/admin/jenis-golongans/create"
          ...
          <table class="w-full caption-bottom text-sm">
            <thead class="[&_tr]:border-b border-border">
              <tr class="border-b transition-colors hover:bg-muted/50 data-[state=selected]:bg-muted">
                <th class="h-12 px-4 text-left align-middle font-medium text-muted-foreground w-3/4">Nama Golongan</th>
                <th class="h-12 px-4 text-right align-middle font-medium text-muted-foreground">Aksi</th>
              </tr>
            </thead>
            <tbody class="[&_tr:last-child]:border-0">
              <tr
                v-for="k in golongans.data"
                :key="k.id"
                class="border-b border-border transition-colors hover:bg-muted/50"
              >
                <td class="p-4 font-medium">{{ k.nama_golongan }}</td>
                <td class="p-4 text-right align-middle">
                  <div class="flex justify-end gap-2">
                    <Link
                      v-if="can('jenis-golongans.edit')"
                      :href="`/admin/jenis-golongans/${k.id}/edit`"
                    ...
                    <button
                      v-if="can('jenis-golongans.delete')"
                      type="button"
                      @click="destroyGolongan(k.id)"
              ...
              <tr v-if="!golongans.data.length">
                <td colspan="2" class="p-8 text-center text-muted-foreground">
                  Tidak ada data Jenis Tindak Golongan ditemukan.
                </td>
              </tr>
            </tbody>
          </table>
      ...
      <div v-if="golongans.links.length > 3" class="flex items-center justify-between mt-2">
        <div class="text-sm text-muted-foreground">
          Menampilkan {{ golongans.from ?? 0 }} sampai {{ golongans.to ?? 0 }} dari {{ golongans.total }} data
        </div>
        <div class="flex gap-1">
          <template v-for="(link, key) in golongans.links" :key="key">
            <div v-if="link.url === null" class="px-3 py-1 text-sm text-muted-foreground border border-transparent" v-html="link.label" />
            <Link v-else :href="link.url" preserve-state preserve-scroll class="px-3 py-1 text-sm rounded-md border transition-colors" :class="link.active ? 'bg-primary text-primary-foreground border-primary' : 'border-border bg-background hover:bg-accent hover:text-accent-foreground bg-card'" v-html="link.label" />
          </template>
        </div>
      </div>
    </div>
