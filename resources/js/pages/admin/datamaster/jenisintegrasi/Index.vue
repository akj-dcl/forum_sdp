...
function destroyIntegrasi(id: number) {
  if (!confirm('Yakin mau hapus Jenis Integrasi ini?')) return
  router.delete(`/admin/jenis-integrasis/${id}`)
}
</script>

<template>
  <Head title="Master Jenis Integrasi" />

  <AppLayout>
    <div class="flex h-full flex-1 flex-col gap-4 p-4 md:p-6 bg-background text-foreground">
      <div class="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
        <div>
          <h1 class="text-2xl font-semibold tracking-tight">Master Jenis Integrasi</h1>
          <p class="text-sm text-muted-foreground">Kelola klasifikasi jenis tindak integrasi Warga Binaan Pemasyarakatan.</p>
        </div>

        <Link
          v-if="can('jenis-integrasis.create')"
          href="/admin/jenis-integrasis/create"
      ...
      <div class="grid gap-3 md:grid-cols-4">
        <input
          v-model="search"
          type="text"
          placeholder="Cari jenis integrasi..."
          ...
          <table class="w-full caption-bottom text-sm">
            <thead class="[&_tr]:border-b border-border">
              <tr class="border-b transition-colors hover:bg-muted/50 data-[state=selected]:bg-muted">
                <th class="h-12 px-4 text-left align-middle font-medium text-muted-foreground w-3/4">Nama Integrasi</th>
                <th class="h-12 px-4 text-right align-middle font-medium text-muted-foreground">Aksi</th>
              </tr>
            </thead>
            <tbody class="[&_tr:last-child]:border-0">
              <tr
                v-for="k in integrasis.data"
                :key="k.id"
                class="border-b border-border transition-colors hover:bg-muted/50"
              >
                <td class="p-4 font-medium">{{ k.nama_integrasi }}</td>
                <td class="p-4 text-right align-middle">
                  <div class="flex justify-end gap-2">
                    <Link
                      v-if="can('jenis-integrasis.edit')"
                      :href="`/admin/jenis-integrasis/${k.id}/edit`"
                    ...
                    <button
                      v-if="can('jenis-integrasis.delete')"
                      type="button"
                      @click="destroyIntegrasi(k.id)"
              ...
              <tr v-if="!integrasis.data.length">
                <td colspan="2" class="p-8 text-center text-muted-foreground">
                  Tidak ada data Jenis Tindak Integrasi ditemukan.
                </td>
              </tr>
            </tbody>
          </table>
      ...
      <div v-if="integrasis.links.length > 3" class="flex items-center justify-between mt-2">
        <div class="text-sm text-muted-foreground">
          Menampilkan {{ integrasis.from ?? 0 }} sampai {{ integrasis.to ?? 0 }} dari {{ integrasis.total }} data
        </div>
        <div class="flex gap-1">
          <template v-for="(link, key) in integrasis.links" :key="key">
