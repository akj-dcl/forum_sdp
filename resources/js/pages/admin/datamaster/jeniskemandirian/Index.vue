...
type JenisKemandirian = {
  id: number
  nama_kemandirian: string
}
type PaginationLink = { url: string | null; label: string; active: boolean }

const props = defineProps<{
  kemandirians: {
    data: JenisKemandirian[]
...
function destroyKemandirian(id: number) {
  if (!confirm('Yakin mau hapus Jenis Kemandirian ini?')) return
  router.delete(`/admin/jenis-kemandirians/${id}`)
}
</script>

<template>
  <Head title="Master Jenis Kemandirian" />

  <AppLayout>
    <div class="flex h-full flex-1 flex-col gap-4 p-4 md:p-6 bg-background text-foreground">
      <div class="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
        <div>
          <h1 class="text-2xl font-semibold tracking-tight">Master Jenis Kemandirian</h1>
          <p class="text-sm text-muted-foreground">Kelola klasifikasi jenis tindak kemandirian Warga Binaan Pemasyarakatan.</p>
        </div>

        <Link
          v-if="can('jenis-kemandirians.create')"
          href="/admin/jenis-kemandirians/create"
      ...
      <div class="grid gap-3 md:grid-cols-4">
        <input
          v-model="search"
          type="text"
          placeholder="Cari jenis kemandirian..."
          ...
          <table class="w-full caption-bottom text-sm">
            <thead class="[&_tr]:border-b border-border">
              <tr class="border-b transition-colors hover:bg-muted/50 data-[state=selected]:bg-muted">
                <th class="h-12 px-4 text-left align-middle font-medium text-muted-foreground w-3/4">Nama Kemandirian</th>
                <th class="h-12 px-4 text-right align-middle font-medium text-muted-foreground">Aksi</th>
              </tr>
            </thead>
            <tbody class="[&_tr:last-child]:border-0">
              <tr
                v-for="k in kemandirians.data"
                :key="k.id"
                class="border-b border-border transition-colors hover:bg-muted/50"
              >
                <td class="p-4 font-medium">{{ k.nama_kemandirian }}</td>
                <td class="p-4 text-right align-middle">
                  <div class="flex justify-end gap-2">
                    <Link
                      v-if="can('jenis-kemandirians.edit')"
                      :href="`/admin/jenis-kemandirians/${k.id}/edit`"
                    ...
                    <button
                      v-if="can('jenis-kemandirians.delete')"
                      type="button"
                      @click="destroyKemandirian(k.id)"
              ...
              <tr v-if="!kemandirians.data.length">
                <td colspan="2" class="p-8 text-center text-muted-foreground">
                  Tidak ada data Jenis Tindak Kemandirian ditemukan.
                </td>
              </tr>
            </tbody>
          </table>
      ...
      <div v-if="kemandirians.links.length > 3" class="flex items-center justify-between mt-2">
        <div class="text-sm text-muted-foreground">
          Menampilkan {{ kemandirians.from ?? 0 }} sampai {{ kemandirians.to ?? 0 }} dari {{ kemandirians.total }} data
        </div>
        <div class="flex gap-1">
          <template v-for="(link, key) in kemandirians.links" :key="key">
