...
const props = defineProps<{
  personel: JenisPersonel
}>()

const form = useForm({
  nama_personel: props.personel.nama_personel ?? '',
})

function submit() {
  form.put(`/admin/jenis-personels/${props.personel.id}`)
}
</script>

<template>
  <Head title="Edit Jenis Tindak Personel" />

  <AppLayout>
    <div class="flex h-full flex-1 flex-col gap-4 p-4 md:p-6 bg-background text-foreground">
      <div class="flex flex-col gap-2 md:flex-row md:items-center md:justify-between">
        <div>
          <h1 class="text-2xl font-semibold tracking-tight">Edit Jenis Tindak Personel</h1>
          <p class="text-sm text-muted-foreground">Perbarui klasifikasi tindak personel.</p>
        </div>

        <Link
          href="/admin/jenis-personels"
        ...
        <form @submit.prevent="submit" class="p-6 space-y-6">
          <div class="grid gap-6 md:grid-cols-2">
            <div class="md:col-span-2 space-y-2">
              <label class="text-sm font-medium leading-none">Nama Personel</label>
              <input
                v-model="form.nama_personel"
                type="text"
              ...
              />
              <div v-if="form.errors.nama_personel" class="text-sm font-medium text-destructive">{{ form.errors.nama_personel }}</div>
            </div>
          </div>
