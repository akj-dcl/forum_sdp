...
const form = useForm({
  nama_pidana: props.pidana.nama_pidana ?? '',
})

function submit() {
  form.put(`/admin/jenis-pidanas/${props.pidana.id}`)
}
</script>

<template>
  <Head title="Edit Jenis Tindak Pidana" />

  <AppLayout>
    <div class="flex h-full flex-1 flex-col gap-4 p-4 md:p-6 bg-background text-foreground">
      <div class="flex flex-col gap-2 md:flex-row md:items-center md:justify-between">
        <div>
          <h1 class="text-2xl font-semibold tracking-tight">Edit Jenis Tindak Pidana</h1>
          <p class="text-sm text-muted-foreground">Perbarui klasifikasi tindak pidana.</p>
        </div>

        <Link
          href="/admin/jenis-pidanas"
        ...
        <form @submit.prevent="submit" class="p-6 space-y-6">
          <div class="grid gap-6 md:grid-cols-2">
            <div class="md:col-span-2 space-y-2">
              ...
              />
              <div v-if="form.errors.nama_pidana" class="text-sm font-medium text-destructive">{{ form.errors.nama_pidana }}</div>
            </div>
          </div>
