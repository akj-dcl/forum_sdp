...
function destroyPidana(id: number) {
  if (!confirm('Yakin mau hapus Jenis Pidana ini?')) return
  router.delete(`/admin/jenis-pidanas/${id}`)
}
</script>

<template>
  <Head title="Master Jenis Pidana" />

  <AppLayout>
    <div class="flex h-full flex-1 flex-col gap-4 p-4 md:p-6 bg-background text-foreground">
      <div class="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
        <div>
          <h1 class="text-2xl font-semibold tracking-tight">Master Jenis Pidana</h1>
          <p class="text-sm text-muted-foreground">Kelola klasifikasi jenis tindak pidana Warga Binaan Pemasyarakatan.</p>
        </div>

        <Link
          v-if="can('jenis-pidanas.create')"
          href="/admin/jenis-pidanas/create"
        ...
        <div class="relative w-full overflow-auto">
          <table class="w-full caption-bottom text-sm">
            ...
            <tbody class="[&_tr:last-child]:border-0">
              <tr
                v-for="k in pidanas.data"
                :key="k.id"
                class="border-b border-border transition-colors hover:bg-muted/50"
              >
                <td class="p-4 font-medium">{{ k.nama_pidana }}</td>
                <td class="p-4 text-right align-middle">
                  <div class="flex justify-end gap-2">
                    <Link
                      v-if="can('jenis-pidanas.edit')"
                      :href="`/admin/jenis-pidanas/${k.id}/edit`"
                    ...
                    <button
                      v-if="can('jenis-pidanas.delete')"
                      type="button"
                      @click="destroyPidana(k.id)"
              ...
              <tr v-if="!pidanas.data.length">
                <td colspan="2" class="p-8 text-center text-muted-foreground">
                  Tidak ada data Jenis Tindak Pidana ditemukan.
                </td>
              </tr>
            </tbody>
          </table>
      ...
      <div v-if="pidanas.links.length > 3" class="flex items-center justify-between mt-2">
        <div class="text-sm text-muted-foreground">
          Menampilkan {{ pidanas.from ?? 0 }} sampai {{ pidanas.to ?? 0 }} dari {{ pidanas.total }} data
        </div>
        <div class="flex gap-1">
          <template v-for="(link, key) in pidanas.links" :key="key">
