...
const form = useForm({
  nama_remisi: props.remisi.nama_remisi ?? '',
})

function submit() {
  form.put(`/admin/jenis-remisis/${props.remisi.id}`)
}
</script>

<template>
  <Head title="Edit Jenis Tindak Remisi" />

  <AppLayout>
    <div class="flex h-full flex-1 flex-col gap-4 p-4 md:p-6 bg-background text-foreground">
      <div class="flex flex-col gap-2 md:flex-row md:items-center md:justify-between">
        <div>
          <h1 class="text-2xl font-semibold tracking-tight">Edit Jenis Tindak Remisi</h1>
          <p class="text-sm text-muted-foreground">Perbarui klasifikasi tindak remisi.</p>
        </div>

        <Link
          href="/admin/jenis-remisis"
        ...
        <form @submit.prevent="submit" class="p-6 space-y-6">
          <div class="grid gap-6 md:grid-cols-2">
            <div class="md:col-span-2 space-y-2">
              <label class="text-sm font-medium leading-none">Nama Remisi</label>
              <input
                v-model="form.nama_remisi"
                type="text"
              ...
              />
              <div v-if="form.errors.nama_remisi" class="text-sm font-medium text-destructive">{{ form.errors.nama_remisi }}</div>
            </div>
          </div>
