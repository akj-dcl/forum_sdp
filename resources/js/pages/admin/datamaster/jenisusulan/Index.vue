...
function destroyUsulan(id: number) {
  if (!confirm('Yakin mau hapus Jenis Usulan ini?')) return
  router.delete(`/admin/jenis-usulans/${id}`)
}
</script>

<template>
  <Head title="Master Jenis Usulan" />

  <AppLayout>
    <div class="flex h-full flex-1 flex-col gap-4 p-4 md:p-6 bg-background text-foreground">
      <div class="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
        <div>
          <h1 class="text-2xl font-semibold tracking-tight">Master Jenis Usulan</h1>
          <p class="text-sm text-muted-foreground">Kelola klasifikasi jenis tindak usulan Warga Binaan Pemasyarakatan.</p>
        </div>

        <Link
          v-if="can('jenis-usulans.create')"
          href="/admin/jenis-usulans/create"
          ...
          <table class="w-full caption-bottom text-sm">
            <thead class="[&_tr]:border-b border-border">
              <tr class="border-b transition-colors hover:bg-muted/50 data-[state=selected]:bg-muted">
                <th class="h-12 px-4 text-left align-middle font-medium text-muted-foreground w-3/4">Nama Usulan</th>
                <th class="h-12 px-4 text-right align-middle font-medium text-muted-foreground">Aksi</th>
              </tr>
            </thead>
            <tbody class="[&_tr:last-child]:border-0">
              <tr
                v-for="k in usulans.data"
                :key="k.id"
                class="border-b border-border transition-colors hover:bg-muted/50"
              >
                <td class="p-4 font-medium">{{ k.nama_usulan }}</td>
                <td class="p-4 text-right align-middle">
                  <div class="flex justify-end gap-2">
                    <Link
                      v-if="can('jenis-usulans.edit')"
                      :href="`/admin/jenis-usulans/${k.id}/edit`"
                    ...
                    <button
                      v-if="can('jenis-usulans.delete')"
                      type="button"
                      @click="destroyUsulan(k.id)"
              ...
              <tr v-if="!usulans.data.length">
                <td colspan="2" class="p-8 text-center text-muted-foreground">
                  Tidak ada data Jenis Tindak Usulan ditemukan.
                </td>
              </tr>
            </tbody>
          </table>
      ...
      <div v-if="usulans.links.length > 3" class="flex items-center justify-between mt-2">
        <div class="text-sm text-muted-foreground">
          Menampilkan {{ usulans.from ?? 0 }} sampai {{ usulans.to ?? 0 }} dari {{ usulans.total }} data
        </div>
        <div class="flex gap-1">
          <template v-for="(link, key) in usulans.links" :key="key">
