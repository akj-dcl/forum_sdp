...
const form = useForm({
  name: props.kanwil.name ?? '',
  code: props.kanwil.code ?? '',
  address: props.kanwil.address ?? '',
  is_active: !!props.kanwil.is_active,
})

function submit() {
  form.put(`/admin/kanwils/${props.kanwil.id}`)
}
</script>

<template>
  <Head title="Edit Kanwil" />
  ...
  <AppLayout>
    <div class="flex h-full flex-1 flex-col gap-4 p-4 md:p-6 bg-background text-foreground dark:bg-inverse-surface">
      <div class="flex flex-col gap-2 md:flex-row md:items-center md:justify-between">
        <div>
          <h1 class="text-2xl font-semibold tracking-tight">Edit Kanwil</h1>
          <p class="text-sm text-muted-foreground">Perbarui data Kantor Wilayah.</p>
        </div>
