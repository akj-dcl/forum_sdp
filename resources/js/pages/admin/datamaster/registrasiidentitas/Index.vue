...
type RegistrasiIdentitas = {
  id: number
  nama_registrasiidentitas: string
}
type PaginationLink = { url: string | null; label: string; active: boolean }

const props = defineProps<{
  registrasiidentitases: {
    data: RegistrasiIdentitas[]
...
function destroyRegistrasiIdentitas(id: number) {
  if (!confirm('Yakin mau hapus Jenis RegistrasiIdentitas ini?')) return
  router.delete(`/admin/registrasi-identitases/${id}`)
}
</script>

<template>
  <Head title="Master Jenis RegistrasiIdentitas" />

  <AppLayout>
    <div class="flex h-full flex-1 flex-col gap-4 p-4 md:p-6 bg-background text-foreground">
      <div class="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
        <div>
          <h1 class="text-2xl font-semibold tracking-tight">Master Jenis RegistrasiIdentitas</h1>
          <p class="text-sm text-muted-foreground">Kelola klasifikasi jenis tindak registrasiidentitas Warga Binaan Pemasyarakatan.</p>
        </div>

        <Link
          v-if="can('registrasi-identitases.create')"
          href="/admin/registrasi-identitases/create"
      ...
      <div class="grid gap-3 md:grid-cols-4">
        <input
          v-model="search"
          type="text"
          placeholder="Cari jenis registrasiidentitas..."
          ...
          <table class="w-full caption-bottom text-sm">
            <thead class="[&_tr]:border-b border-border">
              <tr class="border-b transition-colors hover:bg-muted/50 data-[state=selected]:bg-muted">
                <th class="h-12 px-4 text-left align-middle font-medium text-muted-foreground w-3/4">Nama RegistrasiIdentitas</th>
                <th class="h-12 px-4 text-right align-middle font-medium text-muted-foreground">Aksi</th>
              </tr>
            </thead>
            <tbody class="[&_tr:last-child]:border-0">
              <tr
                v-for="k in registrasiidentitases.data"
                :key="k.id"
                class="border-b border-border transition-colors hover:bg-muted/50"
              >
                <td class="p-4 font-medium">{{ k.nama_registrasiidentitas }}</td>
                <td class="p-4 text-right align-middle">
                  <div class="flex justify-end gap-2">
                    <Link
                      v-if="can('registrasi-identitases.edit')"
                      :href="`/admin/registrasi-identitases/${k.id}/edit`"
                    ...
                    <button
                      v-if="can('registrasi-identitases.delete')"
                      type="button"
                      @click="destroyRegistrasiIdentitas(k.id)"
              ...
              <tr v-if="!registrasiidentitases.data.length">
                <td colspan="2" class="p-8 text-center text-muted-foreground">
                  Tidak ada data Jenis Tindak RegistrasiIdentitas ditemukan.
                </td>
              </tr>
            </tbody>
          </table>
      ...
      <div v-if="registrasiidentitases.links.length > 3" class="flex items-center justify-between mt-2">
        <div class="text-sm text-muted-foreground">
          Menampilkan {{ registrasiidentitases.from ?? 0 }} sampai {{ registrasiidentitases.to ?? 0 }} dari {{ registrasiidentitases.total }} data
        </div>
        <div class="flex gap-1">
          <template v-for="(link, key) in registrasiidentitases.links" :key="key">
