...
const form = useForm({
  nama_registrasipidum: '',
})

function submit() {
  form.post('/admin/registrasi-pidums')
}
</script>

<template>
  <Head title="Tambah Jenis RegistrasiPidum" />

  <AppLayout>
    <div class="flex h-full flex-1 flex-col gap-4 p-4 md:p-6 bg-background text-foreground">
      <div class="flex flex-col gap-2 md:flex-row md:items-center md:justify-between">
        <div>
          <h1 class="text-2xl font-semibold tracking-tight">Tambah Jenis Tindak RegistrasiPidum</h1>
          <p class="text-sm text-muted-foreground">Masukkan klasifikasi tindak registrasipidum baru.</p>
        </div>
