...
const form = useForm({
  kanwil_id: props.kanwils?.[0]?.id ? String(props.kanwils[0].id) : '',
  name: '',
  address: '',
  is_active: true,
})
    ...
    <div class="flex h-full flex-1 flex-col gap-4 p-4 md:p-6 bg-background text-foreground dark:bg-inverse-surface">
      <div class="flex flex-col gap-2 md:flex-row md:items-center md:justify-between">
        <div>
          ...
          <p class="text-sm text-muted-foreground">Isi data Unit Pelaksana Teknis baru.</p>
        </div>
        ...
        <form @submit.prevent="submit" class="p-6 space-y-6">
          <div class="grid gap-6 md:grid-cols-2">
            ...
            <div class="md:col-span-2 space-y-2">
              ...
              <div v-if="form.errors.kanwil_id" class="text-sm font-medium text-destructive">
                {{ form.errors.kanwil_id }}
              </div>
            </div>
            ...
            <div class="md:col-span-2 space-y-2">
              ...
              <input
                v-model="form.name"
                type="text"
                placeholder="Contoh: Lapas Kelas IIA Lombok Barat"
                class="flex h-10 w-full rounded-md border border-input bg-background px-3 py-2 text-sm ring-offset-background focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring disabled:opacity-50"
              />
