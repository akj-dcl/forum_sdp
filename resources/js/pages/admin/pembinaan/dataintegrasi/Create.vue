...
const form = useForm({
  ...
  tanggal_pelaksanaan_pb: '', jumlah_pb: '', sk_pb: null as File | null, dokumentasi_pb: [] as File[],
  tanggal_pelaksanaan_cb: '', jumlah_cb: '', sk_cb: null as File | null, dokumentasi_cb: [] as File[],
  tanggal_pelaksanaan_cmb: '', jumlah_cmb: '', sk_cmb: null as File | null, dokumentasi_cmb: [] as File[],
  tanggal_pelaksanaan_asimilasi: '', jumlah_asimilasi: '', sk_asimilasi: null as File | null, dokumentasi_asimilasi: [] as File[],
  
  // 👇 TAMBAHAN BARU
  tanggal_pelaksanaan_bebas_murni: '', jumlah_bebas_murni: '', sk_bebas_murni: null as File | null, dokumentasi_bebas_murni: [] as File[],
  tanggal_pelaksanaan_cmk: '', jumlah_cmk: '', sk_cmk: null as File | null, dokumentasi_cmk: [] as File[],
})
...
function submit() {
  form.post('/admin/data-integrasis', { forceFormData: true, preserveScroll: true })
}
</script>
  ...
  <AppLayout>
    <div class="flex h-full flex-1 flex-col gap-4 p-4 md:p-6 bg-background text-foreground">
      <div class="flex items-center justify-between mb-4">
        <h1 class="text-2xl font-semibold">Input Data Integrasi WBP</h1>
      ...
      <form @submit.prevent="submit" class="space-y-6">
        <div class="rounded-xl border bg-card p-6 shadow-sm grid md:grid-cols-2 gap-6">
          ...
          <div class="space-y-2">
            ...
            <select v-model="form.upt_id" class="w-full h-10 border rounded-md px-3 bg-background">
        ...
        <div class="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
          ...
          >
            ...
            <div class="space-y-3 bg-muted/30 p-3 rounded-lg border">
              <div>
                <label class="text-xs font-bold text-blue-600 block mb-1">Upload SK (PDF)</label>
                <input type="file" accept="application/pdf" :required="form[`jumlah_${j.key}`] > 0" @input="form[`sk_${j.key}`] = $event.target.files[0]" class="text-xs w-full" />
              </div>
              <div class="pt-2 border-t">
                <label class="text-xs font-bold text-primary block mb-1">Upload Dokumentasi</label>
                <input type="file" multiple accept="image/*" :required="form[`jumlah_${j.key}`] > 0" @change="handleMultipleFiles($event, j.key)" class="text-xs w-full" />
              </div>
            </div>
          </div>
        </div>
        ...
        <div class="text-right pt-4">
          <button type="submit" :disabled="form.processing" class="bg-primary text-primary-foreground px-8 py-3 rounded-md font-bold shadow hover:opacity-90">Simpan Data Integrasi</button>
        </div>
      </form>
    </div>
  </AppLayout>
