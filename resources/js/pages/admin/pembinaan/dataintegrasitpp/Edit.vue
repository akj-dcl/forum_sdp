...
const form = useForm({
  ...
  // 🛠️ PERBAIKAN TYPO DI SINI
  rekomendasi_sidang: props.dataintegrasitpp.rekomendasi_sidang || '',
...
<template>
  <Head title="Edit Sidang TPP" />
  <AppLayout>
    <div class="flex h-full flex-1 flex-col gap-4 p-4 md:p-6 bg-background text-foreground">
      <div class="flex items-center justify-between">
        <h1 class="text-2xl font-semibold">Edit Data Sidang TPP UPT</h1>
      ...
      <div class="rounded-xl border bg-card p-6 shadow-sm">
        <form @submit.prevent="submit" class="grid gap-6 md:grid-cols-2">
            ...
            <div class="md:col-span-2 space-y-2"><label class="text-sm font-medium">Rekomendasi Sidang TPP</label><textarea v-model="form.rekomendasi_sidang" rows="2" class="w-full border rounded-md px-3 py-2 bg-background"></textarea></div>
            ...
            <div class="md:col-span-2 grid md:grid-cols-3 gap-4 p-4 bg-muted/30 rounded-lg border">
                <div class="space-y-2">
                    <label class="text-sm font-bold text-blue-600 block">Berita Acara (Ganti)</label>
                    ...
                    <a v-if="props.dataintegrasitpp.berita_acara" :href="`/view-file?path=${props.dataintegrasitpp.berita_acara}`" target="_blank" class="text-[10px] text-blue-600 hover:underline mt-1 inline-block">📄 Lihat Berita Acara Saat Ini</a>
                </div>
                
                <div class="space-y-2">
                    <label class="text-sm font-bold text-indigo-600 block">Absensi (Ganti)</label>
                    ...
                    <a v-if="props.dataintegrasitpp.absensi" :href="`/view-file?path=${props.dataintegrasitpp.absensi}`" target="_blank" class="text-[10px] text-indigo-600 hover:underline mt-1 inline-block">📋 Lihat Absensi Saat Ini</a>
                </div>
                
                <div class="space-y-2">
                    <label class="text-sm font-bold text-primary block">Dokumentasi (Ganti Semua)</label>
                    <input type="file" multiple accept="image/*" @change="handleFileChange" class="w-full text-xs" />
                    <p class="text-[10px] text-muted-foreground mt-1">Kosongkan jika tidak ingin mengubah {{ props.dataintegrasitpp.dokumentasi_sidang?.length || 0 }} foto saat ini.</p>
                </div>
            </div>

            <div class="md:col-span-2 text-right mt-4"><button type="submit" :disabled="form.processing" class="bg-primary text-primary-foreground px-8 py-2 rounded-md font-bold shadow hover:opacity-90">Simpan Perubahan TPP</button></div>
        </form>
      </div>
    </div>
  </AppLayout>
