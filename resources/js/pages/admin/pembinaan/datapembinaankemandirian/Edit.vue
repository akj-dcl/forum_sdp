...
const form = useForm({
  _method: 'put',
  tanggal: props.datapembinaankemandirians.tanggal || '',
  upt_id: String(props.datapembinaankemandirians.upt_id) || '',
  jenis_kemandirian_id: String(props.datapembinaankemandirians.jenis_kemandirian_id) || '',
  detail_lain_lain: props.datapembinaankemandirians.detail_lain_lain || '', // Data Baru
  nama_kegiatan: props.datapembinaankemandirians.nama_kegiatan || '',
  jumlah_peserta: props.datapembinaankemandirians.jumlah_peserta || '',
  hasil_kegiatan: props.datapembinaankemandirians.hasil_kegiatan || '',
  rekomendasi_kegiatan: props.datapembinaankemandirians.rekomendasi_kegiatan || '',
...
function submit() {
  ...
  form.post(`/admin/data-pembinaan-kemandirians/${props.datapembinaankemandirians.id}`)
}
</script>
  ...
  <AppLayout>
    <div class="flex h-full flex-1 flex-col gap-4 p-4 md:p-6 bg-background text-foreground">
      <div class="flex flex-col gap-2 md:flex-row md:items-center md:justify-between">
        <div>
          <h1 class="text-2xl font-semibold tracking-tight">Edit Kegiatan Kemandirian</h1>
        </div>
        ...
        <form @submit.prevent="submit" class="p-6 space-y-6">
          <div class="grid gap-6 md:grid-cols-2">
            ...
            <div v-if="isLainLain" class="md:col-span-2 space-y-2 bg-orange-50/50 border border-orange-200 p-4 rounded-lg animate-in fade-in zoom-in duration-200">
              ...
              <input v-model="form.detail_lain_lain" type="text" required class="flex h-10 w-full rounded-md border border-orange-300 bg-white px-3 py-2 text-sm focus:ring-orange-500" />
            </div>
