...
const form = useForm({
  _method: 'put',
  tanggal: props.datapembinaankepribadians.tanggal || '',
  upt_id: String(props.datapembinaankepribadians.upt_id) || '',
  jenis_kepribadian_id: String(props.datapembinaankepribadians.jenis_kepribadian_id) || '',
  detail_lain_lain: props.datapembinaankepribadians.detail_lain_lain || '', // Data Baru
  nama_kegiatan: props.datapembinaankepribadians.nama_kegiatan || '',
  pemateri: props.datapembinaankepribadians.pemateri || '',
  jumlah_peserta: props.datapembinaankepribadians.jumlah_peserta || '',
  hasil_kegiatan: props.datapembinaankepribadians.hasil_kegiatan || '',
  rekomendasi_kegiatan: props.datapembinaankepribadians.rekomendasi_kegiatan || '',
  ...
  <AppLayout>
    <div class="flex h-full flex-1 flex-col gap-4 p-4 md:p-6 bg-background text-foreground">
      <div class="flex flex-col gap-2 md:flex-row md:items-center md:justify-between">
        <div>
          <h1 class="text-2xl font-semibold tracking-tight">Edit Kegiatan Kepribadian</h1>
        </div>
        ...
        <form @submit.prevent="submit" class="p-6 space-y-6">
          <div class="grid gap-6 md:grid-cols-2">
            ...
            <div class="space-y-2">
              ...
              <input v-model="form.nama_kegiatan" type="text" required class="flex h-10 w-full rounded-md border border-input bg-background px-3 py-2 text-sm ring-offset-background disabled:opacity-50" />
            </div>
            ...
            <div v-if="isLainLain" class="md:col-span-2 space-y-2 bg-blue-50/50 border border-blue-200 p-4 rounded-lg animate-in fade-in zoom-in duration-200">
              ...
              <input v-model="form.detail_lain_lain" type="text" required class="flex h-10 w-full rounded-md border border-blue-300 bg-white px-3 py-2 text-sm focus:ring-blue-500" />
            </div>
            ...
            <div class="md:col-span-2 space-y-2 bg-muted/30 p-4 rounded-lg border border-border">
              <label class="text-sm font-semibold leading-none">Upload Ulang Dokumentasi?</label>
              <p class="text-xs text-muted-foreground mb-2">Kosongkan saja jika tidak ingin mengganti file dokumentasi yang lama. Jika diisi, file lama akan dihapus.</p>
              
              <input type="file" multiple accept="image/*,video/mp4,video/quicktime" @change="handleFileChange" class="flex w-full rounded-md border border-input bg-background px-3 py-2 text-sm file:border-0 file:bg-transparent file:text-sm file:font-medium hover:cursor-pointer" />
            </div>
