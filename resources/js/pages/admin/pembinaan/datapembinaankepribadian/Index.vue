...
type JenisKepribadian = { id: number; nama_kepribadian: string }
...
type DataKepribadian = {
  ...
  upt?: Upt
  jenis_kepribadian?: JenisKepribadian
}

const props = defineProps<{
  datapembinaankepribadians: { data: DataKepribadian[], links: any[], from: number, to: number, total: number }
  upts: Upt[]
...
const selectedData = ref<DataKepribadian | null>(null)

function showDetail(data: DataKepribadian) {
...
function destroyData(id: number) {
  ...
  router.delete(`/admin/data-pembinaan-kepribadians/${id}`)
}
</script>

<template>
  <Head title="Pembinaan Kepribadian" />

  <AppLayout>
    <div class="flex h-full flex-1 flex-col gap-4 p-4 md:p-6 bg-background text-foreground">
      <div class="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
        <div>
          <h1 class="text-2xl font-semibold tracking-tight">Pembinaan Kepribadian</h1>
          <p class="text-sm text-muted-foreground">Kelola data kegiatan kepribadian</p>
        </div>
        <Link v-if="can('PembinaanKepribadians.create')" href="/admin/data-pembinaan-kepribadians/create" class="inline-flex items-center justify-center rounded-md bg-primary px-4 py-2 text-sm font-medium text-primary-foreground shadow hover:opacity-90 transition-colors">+ Tambah Kegiatan</Link>
      </div>
        ...
        <div class="relative w-full overflow-auto">
          <table class="w-full caption-bottom text-sm">
            ...
            <tbody class="[&_tr:last-child]:border-0">
              <tr v-for="d in datapembinaankepribadians.data" :key="d.id" class="border-b border-border transition-colors hover:bg-muted/50">
                ...
                <td class="p-4">
                  <span class="inline-flex items-center rounded-full bg-blue-100 px-2.5 py-0.5 text-xs font-semibold text-blue-700 dark:bg-blue-900/30 dark:text-blue-400">
                    {{ d.jenis_kepribadian?.nama_kepribadian ?? '-' }} 
                  </span>
                </td>
                <td class="p-4 text-right align-middle">
                  <div class="flex justify-end gap-2">
                    ...
                    <Link v-if="can('PembinaanKepribadians.edit')" :href="`/admin/data-pembinaan-kepribadians/${d.id}/edit`" class="inline-flex items-center justify-center rounded-md text-sm font-medium transition-colors border border-input bg-transparent shadow-sm hover:bg-accent hover:text-accent-foreground h-8 px-3">Edit</Link>
                    <button v-if="can('PembinaanKepribadians.delete')"  type="button" @click="destroyData(d.id)" class="inline-flex items-center justify-center rounded-md text-sm font-medium transition-colors bg-destructive text-destructive-foreground shadow-sm hover:bg-destructive/90 h-8 px-3">Hapus</button>
                  </div>
                </td>
              </tr>
              <tr v-if="!datapembinaankepribadians.data.length">
      ...
      <div v-if="datapembinaankepribadians.links.length > 3" class="flex items-center justify-between mt-2">
        <div class="text-sm text-muted-foreground">Menampilkan {{ datapembinaankepribadians.from ?? 0 }} sampai {{ datapembinaankepribadians.to ?? 0 }} dari {{ datapembinaankepribadians.total }} data</div>
        <div class="flex gap-1">
          <template v-for="(link, key) in datapembinaankepribadians.links" :key="key">
    ...
    <div v-if="isModalOpen" class="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4 backdrop-blur-sm transition-opacity" @click.self="closeModal">
      <div class="w-full max-w-3xl max-h-[90vh] overflow-y-auto rounded-xl border border-border bg-card p-6 text-card-foreground shadow-lg">
        <div class="flex items-center justify-between border-b pb-3 mb-4 sticky top-0 bg-card z-10">
          <h2 class="text-lg font-semibold">Detail Kegiatan Kepribadian</h2>
        ...
        <div v-if="selectedData" class="grid gap-4 md:grid-cols-2 text-sm">
          ...
          <div><p class="text-muted-foreground text-xs mb-1">Jenis Kepribadian</p><p class="font-medium">{{ selectedData.jenis_kepribadian?.nama_kepribadian ?? '-' }}</p></div><div>
              <p class="text-muted-foreground text-xs mb-1">Jenis Kepribadian</p>
              <p class="font-medium">
                  {{ selectedData.jenis_kepribadian?.nama_kepribadian ?? '-' }}
                  <span v-if="selectedData.detail_lain_lain" class="text-xs font-bold text-blue-600 bg-blue-50 px-2 py-0.5 rounded border border-blue-200 ml-1">
                      ({{ selectedData.detail_lain_lain }})
                  </span>
              </p>
          </div>
          <div><p class="text-muted-foreground text-xs mb-1">Pemateri</p><p class="font-medium">{{ selectedData.pemateri }}</p></div>
