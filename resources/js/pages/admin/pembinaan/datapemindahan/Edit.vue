...
const form = useForm({
    ...
    upt_id: String(props.datapemindahan.upt_id),

    tanggal_input: props.datapemindahan.tanggal_input,

    tanggal_pelaksanaan: props.datapemindahan.tanggal_pelaksanaan,

    upt_asal_id: String(props.datapemindahan.upt_asal_id),

    upt_tujuan_id: props.datapemindahan.upt_tujuan_id
        ? String(props.datapemindahan.upt_tujuan_id)
        : '',

    is_keluar_wilayah: props.datapemindahan.is_keluar_wilayah ?? false,

    tujuan_luar_wilayah: props.datapemindahan.tujuan_luar_wilayah ?? '',

    jenis_pemindahan_id: String(props.datapemindahan.jenis_pemindahan_id),

    detail_lain_lain: props.datapemindahan.detail_lain_lain ?? '',

    jenis_kasus: props.datapemindahan.jenis_kasus ?? '',
    ...
    jumlah_personel: props.datapemindahan.jumlah_personel || {}
})
...
onMounted(() => {
    ...
    props.jenis_personels?.forEach(item => {
        ...
        if (form.jumlah_personel[item.id] === undefined) {
...
function submit() {
    ...
    form.post(`/admin/data-pemindahans/${props.datapemindahan.id}`, {
        forceFormData: true
    })
}
</script>
                    ...
                    <div class="rounded-xl border bg-card p-6 shadow-sm space-y-6">
                        ...
                        <div class="bg-blue-50 p-4 rounded-lg border border-blue-100">
                            ...
                            <input
                                ...
                                @input="form.surat_usulan = $event.target.files[0]"
                        ...
                        <div class="bg-indigo-50 p-4 rounded-lg border border-indigo-100">
                            ...
                            <input
                                type="file"
                                accept="application/pdf"
                                @input="form.surat_persetujuan = $event.target.files[0]"
                                class="text-sm w-full"
                            />
                        </div>
                        ...
                        <div class="bg-orange-50 p-4 rounded-lg border border-orange-100">
                            ...
                            <input
                                ...
                                multiple
                                accept="image/*"
                                @input="form.foto_pemindahan = Array.from($event.target.files)"
                                class="text-sm w-full"
                            />
                        </div>
                    </div>
