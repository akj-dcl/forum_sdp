...
const props = defineProps<{ dataremisi: any, upts: any[], jenis_remisis: any[] }>()

const form = useForm({
  _method: 'put',
  upt_id: String(props.dataremisi.upt_id),
  tanggal_input: props.dataremisi.tanggal_input,
  // Mengambil JSON array langsung dari database
  detail_remisi: props.dataremisi.detail_remisi || []
})

function addRow() {
    form.detail_remisi.push({ jenis_remisi_id: '', jumlah_diusulkan: 0, jumlah_remisi: 0, jumlah_selisih: '0', keterangan: '', sk_remisi: null })
}
...
function submit() {
  form.post(`/admin/data-remisis/${props.dataremisi.id}`, {
  forceFormData: true
})
}
</script>
...
<template>
  <Head title="Edit Remisi Harian" />
  <AppLayout>
    <div class="flex h-full flex-1 flex-col gap-4 p-4 md:p-6 bg-background text-foreground">
      <div class="flex items-center justify-between mb-2">
        <div>
            <h1 class="text-2xl font-semibold tracking-tight">Edit Data Remisi Harian</h1>
            <p class="text-sm text-muted-foreground">Perbarui pemberian jenis remisi pada tanggal ini.</p>
        </div>
