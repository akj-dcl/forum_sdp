...
const props = defineProps<{ upts: Upt[], jenis_pidanas: JenisPidana[] }>()
...
const form = useForm({
  ...
  jenis_pidana_sekarang_id: props.jenis_pidanas?.[0]?.id ? String(props.jenis_pidanas[0].id) : '',
  lama_pidana_sekarang: '',
  tempat_pidana_sekarang: '',
  
  berapa_kali_dipidana: 1,
  putusan_pengadilan: null as File | null,
  
  jenis_pidana_sebelumnya_id: props.jenis_pidanas?.[0]?.id ? String(props.jenis_pidanas[0].id) : '',
  lama_pidana_sebelumnya: '',
  tempat_pidana_sebelumnya: '',
  jenis_pembebasan_sebelumnya: '',
})
...
<template>
  <Head title="Tambah Data Residivis" />
  <AppLayout>
    <div class="flex h-full flex-1 flex-col gap-4 p-4 md:p-6 bg-background text-foreground">
      <div class="flex flex-col gap-2 md:flex-row md:items-center md:justify-between mb-4">
        <div>
          <h1 class="text-2xl font-semibold tracking-tight text-primary">Tambah Data Residivis</h1>
          <p class="text-sm text-muted-foreground">Input data riwayat pengulangan tindak pidana WBP.</p>
        </div>
      ...
      <form @submit.prevent="submit" class="space-y-6">
          ...
          <div class="grid lg:grid-cols-2 gap-6">
              <div class="rounded-xl border border-red-200 bg-red-50/30 p-6 shadow-sm space-y-4">
                  ...
                  <div class="grid grid-cols-2 gap-4 pt-2 border-t border-red-200/50">
                      ...
                      <div class="space-y-2">
                          <label class="text-xs font-bold text-blue-600 uppercase">Upload Putusan Pengadilan</label>
          ...
          <div class="flex justify-end pt-4">
            <button type="submit" :disabled="form.processing" class="inline-flex items-center justify-center rounded-xl bg-primary px-10 py-3 text-sm font-bold text-primary-foreground shadow hover:opacity-90">Simpan Data Residivis</button>
          </div>
      </form>
    </div>
  </AppLayout>
