...
const props = defineProps<{ datapengawalanpengamanan: any, upts: any[], jenis_pengawalans: any[], jenis_klasifikasis: any[] }>()

const form = useForm({
  _method: 'put',
  upt_id: String(props.datapengawalanpengamanan.upt_id),
  tanggal_input: props.datapengawalanpengamanan.tanggal_input,
  tanggal_pelaksanaan: props.datapengawalanpengamanan.tanggal_pelaksanaan,
  waktu_pelaksanaan_mulai: props.datapengawalanpengamanan.waktu_pelaksanaan_mulai,
  waktu_pelaksanaan_selesai: props.datapengawalanpengamanan.waktu_pelaksanaan_selesai,
  jenis_pengawalan_id: String(props.datapengawalanpengamanan.jenis_pengawalan_id),
  jenis_klasifikasi_id: String(props.datapengawalanpengamanan.jenis_klasifikasi_id),
  sarana_pengawalan: props.datapengawalanpengamanan.sarana_pengawalan,
  
  jumlah_wbp: props.datapengawalanpengamanan.jumlah_wbp,
  detail_wbp_dikawal: props.datapengawalanpengamanan.detail_wbp_dikawal || [],
  
  jumlah_petugas: props.datapengawalanpengamanan.jumlah_petugas,
  detail_petugas_pengawalan: props.datapengawalanpengamanan.detail_petugas_pengawalan || [],
...
function submit() {
  form.post(`/admin/data-pengawalan-pengamanans/${props.datapengawalanpengamanan.id}`, { forceFormData: true })
}
</script>
      ...
      <form @submit.prevent="submit" class="space-y-6">
        ...
        <div class="grid md:grid-cols-3 gap-4">
            <div class="rounded-xl border bg-orange-50 p-4 shadow-sm border-orange-100 flex flex-col justify-center">
                ...
                <input type="file" accept="application/pdf" @input="form.surat_perintah = $event.target.files[0]" class="text-xs mt-2 file:mr-2 file:rounded file:border-0 file:bg-orange-600 file:text-white file:px-3 file:py-1 hover:file:bg-orange-700 cursor-pointer" />
            </div>
            ...
            <div class="rounded-xl border bg-blue-50 p-4 shadow-sm border-blue-100 flex flex-col justify-center">
                ...
                <input type="file" accept="application/pdf" @input="form.laporan_pelaksanaan_pengawalan = $event.target.files[0]" class="text-xs mt-2 file:mr-2 file:rounded file:border-0 file:bg-blue-600 file:text-white file:px-3 file:py-1 hover:file:bg-blue-700 cursor-pointer" />
            </div>
            ...
            <div class="rounded-xl border bg-indigo-50 p-4 shadow-sm border-indigo-100 flex flex-col justify-center">
                ...
                <input type="file" accept="image/*" @input="form.dokumentasi = $event.target.files[0]" class="text-xs mt-2 file:mr-2 file:rounded file:border-0 file:bg-indigo-600 file:text-white file:px-3 file:py-1 hover:file:bg-indigo-700 cursor-pointer" />
            </div>
        </div>
