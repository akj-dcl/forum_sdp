...
const props = defineProps<{ upts: any[], jenis_pengeluarans: any[] }>()
...
const form = useForm({
  ...
  // Kekuatan Pengamanan
  nama_regu: '', jumlah_regu: 0, jumlah_hadir: 0, jumlah_tidak_hadir: 0,
  keterangan: '', perwira_piket: '', jumlah_bantuan_piket: 0,
  pengawalan_rs: '', patroli_sambang: '', waktu_patroli_sambang: '',

  // Data Hunian
  jumlah_kapasitas: 0,
  jumlah_napi_laki: 0, jumlah_napi_perempuan: 0, jumlah_napi_anak: 0,
  jumlah_tahanan_laki: 0, jumlah_tahanan_perempuan: 0, jumlah_tahanan_anak: 0,
  total_isi: 0, overcrowded: 0,

  // Pengeluaran
  jumlah_pengeluaran: {} as Record<number, number>
})

onMounted(() => {
    props.jenis_pengeluarans?.forEach(item => { form.jumlah_pengeluaran[item.id] = 0 })
})

// Sihir Auto Hitung Hunian
...
function submit() {
  form.post('/admin/data-penghuni-riils')
}
</script>
  ...
  <AppLayout>
    <div class="flex h-full flex-1 flex-col gap-4 p-4 md:p-6 bg-background text-foreground">
      ...
      <form @submit.prevent="submit" class="space-y-6">
        ...
        <div class="text-right"><button type="submit" :disabled="form.processing" class="bg-primary text-primary-foreground px-10 py-3 rounded-md font-bold shadow-lg hover:opacity-90">Simpan Laporan Papan Portir</button></div>
      </form>
    </div>
  </AppLayout>
</template>