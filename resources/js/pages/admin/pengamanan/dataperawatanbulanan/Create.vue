...
const form = useForm({
  ...
  jumlah_penghuni_hiv: 0,
  jumlah_penghuni_tbc: 0,
  jumlah_penghuni_disabilitas: 0,
  jumlah_penghuni_jiwa: 0,
  jumlah_penghuni_tidakmenular: 0,
  jumlah_penghuni_menular: 0,
  jenis_penyakit_dominan: '',
  jumlah_wbp_lansia: 0,
  jumlah_wbp_berkepanjangan: 0,
  jumlah_peserta_bpjs: 0,
  laporan: null as File | null,
})

function submit() {
  form.post('/admin/data-perawatan-bulanans')
}
</script>

<template>
  <Head title="Input Perawatan Bulanan" />
  <AppLayout>
    <div class="flex h-full flex-1 flex-col gap-4 p-4 md:p-6 bg-background text-foreground">
      <div class="flex items-center justify-between mb-2">
        <h1 class="text-2xl font-semibold">Input Data Perawatan Bulanan</h1>
      ...
      <form @submit.prevent="submit" class="space-y-6">
        ...
        <div class="rounded-xl border bg-card p-6 shadow-sm grid md:grid-cols-2 gap-6">
            <div class="space-y-2"><label class="text-sm font-bold">Bulan Laporan (Pilih Tanggal Akhir Bulan)</label><input v-model="form.tanggal_input" type="date" class="w-full h-10 border rounded-md px-3 bg-background" /></div>
        ...
        <div class="grid md:grid-cols-2 gap-6">
            <div class="rounded-xl border bg-card p-5 shadow-sm space-y-4">
                ...
                <div class="space-y-2 mt-4 pt-2 border-t">
                    <label class="text-xs font-bold text-primary">Jenis Penyakit Dominan (Terbanyak Bulan Ini)</label>
                    <input v-model="form.jenis_penyakit_dominan" type="text" placeholder="Contoh: ISPA, Scabies, Kulit..." class="w-full h-9 border rounded px-3 text-sm bg-background" required />
                </div>
            </div>
            ...
            <div class="rounded-xl border bg-card p-5 shadow-sm space-y-4">
                ...
                <div class="space-y-2 mt-4 pt-2 border-t">
                    ...
                    <input v-model.number="form.jumlah_peserta_bpjs" type="number" min="0" placeholder="Jumlah Peserta BPJS" class="w-full h-9 border rounded px-3 text-sm bg-blue-50 font-bold text-blue-700" />
                </div>
            </div>
        </div>
        ...
        <div class="rounded-xl border bg-blue-50 p-6 shadow-sm border-blue-100 flex items-center justify-between">
            <div>
                <h3 class="font-bold text-blue-800">Upload Laporan Bulanan (PDF)</h3>
                <p class="text-xs text-blue-600 mt-1">Laporan rekap bulanan yang sudah ditandatangani Kepala UPT.</p>
            </div>
            <input type="file" accept="application/pdf" @input="form.laporan = $event.target.files[0]" required class="text-sm file:mr-3 file:rounded file:border-0 file:bg-blue-600 file:text-white file:px-4 file:py-2 hover:file:bg-blue-700 cursor-pointer" />
        </div>

        <div class="text-right"><button type="submit" :disabled="form.processing" class="bg-primary text-primary-foreground px-8 py-3 rounded-md font-bold shadow hover:opacity-90">Simpan Perawatan Bulanan</button></div>
      </form>
    </div>
  </AppLayout>
</template>