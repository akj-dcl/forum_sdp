...
const props = defineProps<{ dataregupengamanan: any, upts: any[] }>()

const form = useForm({
  _method: 'put',
  upt_id: String(props.dataregupengamanan.upt_id),
  tanggal_input: props.dataregupengamanan.tanggal_input,
  shift_rupam: props.dataregupengamanan.shift_rupam || '',
  nama_regu: props.dataregupengamanan.nama_regu,
  waktu_tugas_mulai: props.dataregupengamanan.waktu_tugas_mulai,
  waktu_tugas_selesai: props.dataregupengamanan.waktu_tugas_selesai,
  jumlah_regu_jaga: props.dataregupengamanan.jumlah_regu_jaga,
  jumlah_hadir: props.dataregupengamanan.jumlah_hadir,
  jumlah_tidak_hadir: props.dataregupengamanan.jumlah_tidak_hadir,
  nama_komandan_jaga: props.dataregupengamanan.nama_komandan_jaga || '',
  nama_wadan_jaga: props.dataregupengamanan.nama_wadan_jaga || '',
  nama_p2u: props.dataregupengamanan.nama_p2u || '',
  nama_petugas_penjagaan: props.dataregupengamanan.nama_petugas_penjagaan || '',
  jumlah_petugas_luar: props.dataregupengamanan.jumlah_petugas_luar,
  jenis_bantuan: props.dataregupengamanan.jenis_bantuan || '',
  jumlah_bantuan: props.dataregupengamanan.jumlah_bantuan,
  jumlah_titik_pos: props.dataregupengamanan.jumlah_titik_pos,
  pos_pengamanan_diisi: props.dataregupengamanan.pos_pengamanan_diisi || '',
  pos_pengamanan_tidak_diisi: props.dataregupengamanan.pos_pengamanan_tidak_diisi || '',
  dokumentasi: null as File | null,
})

function submit() {
  form.post(`/admin/data-regu-pengamanans/${props.dataregupengamanan.id}`, { forceFormData: true })
}
</script>
  ...
  <AppLayout>
    <div class="flex h-full flex-1 flex-col gap-4 p-4 md:p-6 bg-background text-foreground">
      ...
      <form @submit.prevent="submit" class="space-y-6">
        ...
        <div class="text-right"><button type="submit" :disabled="form.processing" class="bg-primary text-primary-foreground px-8 py-3 rounded-md font-bold shadow-lg hover:opacity-90">Simpan Perubahan Laporan Regu</button></div>
      </form>
    </div>
  </AppLayout>
</template>